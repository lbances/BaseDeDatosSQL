...
create index NombreEmpleado on Cargo_Empleado(NombreC);

create synonym CargoX for Cargo_Empleado;
...
SELECT T.AP_TRA, T.AM_TRA , T.NOM_TRA, T.SUELDO,
       T.SUELDO*0.09 ESSALUD,T.SUELDO*0.13 ONP, T.SUELDO*0.87 PagoTotal
FROM TRABAJADOR T
...
SELECT L.AREALIB, SUM(L.PRECIOLIB)
FROM LIBRO L
GROUP BY L.AREALIB HAVING L.AREALIB IN ('ARTE','PROYECTOS','LPROG');

SELECT L.TITULOLIB, L.AREALIB, L.PRECIOLIB,
       L.PRECIOLIB * 0.18 IGV, L.PRECIOLIB + (L.PRECIOLIB * 0.18) TOTALPAGAR
FROM LIBRO L
WHERE L.AREALIB IN ('LPROG');

SELECT A.NOMBRE C.DESCRIPCION, N.NOTA1, N.NOTA2, N.NOTA3, N.NOTA4,
