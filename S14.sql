...
BEGIN
    ...
    INTO Codigo, Titulo, Autor, Editorial, Area,
         Precio, CantidadHojas, AnnoLibro FROM LIBRO 
    WHERE CODLIB = &Codigo; 
...
CREATE OR REPLACE PROCEDURE GRABAR_ALUMNO
                (Codigo Alumno.Cod_Alu%Type,
                 ...
                 Grado Alumno.Grado%Type,
                 FechaNacimiento Alumno.F_Nacimiento%Type,
	               Delegado Alumno.Delegado%Type,
                 Edad Alumno.Edad%Type,
                 Estado Alumno.Estado%Type);
IS
BEGIN
      INSERT INTO ALUMNO VALUES (Codigo, Nombre, Sexo, Grado, FechaNacimiento, Delegado, Edad, Estado);
      COMMIT;
END;
...
BEGIN
     ...
     DBMS_OUTPUT.PUT_LINE('Delegado : ' || Delegado);
...
-- Creación del paquete Alumno

CREATE OR REPLACE PACKAGE paqueteAlumno
IS
...
CREATE OR REPLACE PACKAGE BODY paqueteAlumno
IS
      -- PROCEDIMIENTO
      -- Alumno
      PROCEDURE GRABAR_ALUMNO
                (Codigo Alumno.Cod_Alu%Type,
                 ...
                 Grado Alumno.Grado%Type,
                 FechaNacimiento Alumno.F_Nacimiento%Type,
	       Delegado Delegado.Delegado%Type,
                 Edad Alumno.Edad%Type,
                 Estado Alumno.Estado%Type);
...
CREATE OR REPLACE TRIGGER DUPLICAR_ALUMNO
...
BEGIN
	INSERT INTO LISTA_ALUMNOS(Cod_Alu, Nombre, Sexo, Grado, F_Nacimiento, Delegado, Edad, Estado)
	VALUES(:NEW.Cod_Alu, :NEW.Nombre, :NEW.Sexo, :NEW.Grado, :NEW.F_Nacimiento, :NEW.Delegado, :NEW.Edad, :NEW.Estado)
END;

-- Disparador 2
CREATE OR REPLACE TRIGGER DELEGADO
...
BEGIN
	SELECT COUNT(*) INTO Delegar FROM Alumno WHERE DELEGADO = :NEW.DELEGADO;
	IF (Delegar > 4) THEN
	raise_application_error(-20600, :NEW.DELEGADO || 'No puede delegar mas de 5 alumnos.');
	END IF;
END;

-- Disparador 3
CREATE OR REPLACE TRIGGER JEFE
...
BEGIN
	SELECT COUNT(*) INTO Lider FROM Trabajador WHERE JEFE = :NEW.JEFE;
	IF (Lider > 9) THEN
	raise_application_error(-20600, :NEW.JEFE || 'No puede tener mas de 10 trabajadores a su cargo.');
	END IF;
END;
