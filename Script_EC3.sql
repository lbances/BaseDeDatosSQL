...
-- Sueldo Máximo, se obtendrá de la tabla TRABAJADOR.
CREATE OR REPLACE FUNCTION SUELDO_MAX  
RETURN FLOAT
IS
    TOTAL FLOAT:=0;
BEGIN
    SELECT MAX (SUELDO) INTO TOTAL FROM TRABAJADOR;
    RETURN TOTAL;
END;

SET SERVEROUTPUT ON;
BEGIN
    DBMS_OUTPUT.PUT_LINE('Sueldo máximo: ' || SUELDO_MAX);
END;


-- Sueldo Mínimo, se obtendrá de la tabla TRABAJADOR.
CREATE OR REPLACE FUNCTION SUELDO_MIN
RETURN FLOAT
IS
    TOTAL FLOAT:=0;
BEGIN
    SELECT MIN(SUELDO) INTO TOTAL FROM TRABAJADOR;
    RETURN TOTAL;
END;

SET SERVEROUTPUT ON;
BEGIN
    DBMS_OUTPUT.PUT_LINE('Sueldo mínimo: ' || SUELDO_MIN);
END;


--Totalización de sueldos según Genero, se obtendrá de la tabla TRABAJADOR.
CREATE OR REPLACE FUNCTION TOTAL_HOMBRE
       (Genero Varchar2)
RETURN INT
IS
    TOTAL INT:=0;
BEGIN
    SELECT SUM (SUELDO) INTO TOTAL FROM TRABAJADOR WHERE GENERO_TRA = Genero;
    RETURN TOTAL;
END;
...
DECLARE
  Ge Varchar2 (20):='M';
BEGIN
    DBMS_OUTPUT.PUT_LINE('Sueldo masculino: ' || TOTAL_HOMBRE('M'));
    DBMS_OUTPUT.PUT_LINE('Sueldo femenino: ' || TOTAL_HOMBRE('F'));
END;


--Costo total de los libros, se obtendrá de la tabla LIBRO.
CREATE OR REPLACE FUNCTION TOTAL_LIBROS
...
BEGIN
DBMS_OUTPUT.PUT_LINE('Total de los libros: ' || TOTAL_LIBROS);
END;


-- Costo total según área del libro, se obtendrá de la tabla LIBRO.
CREATE OR REPLACE FUNCTION TOTAL_LIBROS
       (Area Varchar2)
RETURN INT
IS
...
BEGIN
    DBMS_OUTPUT.PUT_LINE('Area: LPROG     Son: ' || TOTAL_LIBROS('LPROG'));
    DBMS_OUTPUT.PUT_LINE('Area: ANALISIS  Son: ' || TOTAL_LIBROS('ANALISIS'));
    DBMS_OUTPUT.PUT_LINE('Area: CAD       Son: ' || TOTAL_LIBROS('CAD'));
    DBMS_OUTPUT.PUT_LINE('Area: ARTE      Son: ' || TOTAL_LIBROS('ARTE'));
    DBMS_OUTPUT.PUT_LINE('Area: LIDERAZGO Son: ' || TOTAL_LIBROS('LIDERAZGO'));
    DBMS_OUTPUT.PUT_LINE('Area: PROYECTOS Son: ' || TOTAL_LIBROS('PROYECTOS'));
END;
...
--PROCEDIMIENTO TABLA ALUMNO
-- GRABAR
CREATE OR REPLACE PROCEDURE GRABAR_ALUMNO
(Codigo Alumno.Cod_Alu%Type, Nombre Alumno.Nombre%Type,
 Sexo Alumno.Sexo%Type, Grado Alumno.Grado%Type,
 FechaNacimiento Alumno.F_Nacimiento%Type, Edad Alumno.Edad%Type,
 Estado Alumno.Estado%Type)
IS
BEGIN
      INSERT INTO ALUMNO VALUES (Codigo, Nombre, Sexo, Grado, FechaNacimiento, Edad, Estado);
      COMMIT;
END;

-- CONSULTAR
CREATE OR REPLACE PROCEDURE CONSULTAR_ALUMNO
(Codigo Alumno.Cod_Alu%Type)
IS
   Nombre Alumno.Nombre%Type;
   Sexo Alumno.Sexo%Type; Gr Alumno.Grado%Type;
   FechaNacimiento Alumno.F_Nacimiento%Type; Ed Alumno.Edad%Type;
   Estado Alumno.Estado%Type;
BEGIN
     SELECT Nombre, Sexo, Grado, F_Nacimiento, Edad, Estado 
     INTO Nombre, Sexo, Grado, FechaNacimiento, Edad, Estado FROM ALUMNO
     WHERE Cod_Alu = Codigo;
     DBMS_OUTPUT.PUT_LINE('================================');
     DBMS_OUTPUT.PUT_LINE('Datos del Alumno: ');
     DBMS_OUTPUT.PUT_LINE('Codigo           : ' || Codigo);
     DBMS_OUTPUT.PUT_LINE('Nombre           : ' || Nombre);
     DBMS_OUTPUT.PUT_LINE('Género           : ' || Sexo);
     DBMS_OUTPUT.PUT_LINE('Grado            : ' || Grado);
     DBMS_OUTPUT.PUT_LINE('Fecha Nacimiento : ' || FechaNacimiento);
...
IS
BEGIN
     DELETE FROM ALUMNO WHERE COD_ALU = Codigo;
     COMMIT;
END;
...
IS
BEGIN
     UPDATE ALUMNO SET Nombre = Nombre, Sexo = Sexo,
                       Grado = Grado, F_Nacimiento = FechaNacimiento,
...
--PROCEDIMIENTO CURSOS
--  GRABAR
CREATE OR REPLACE PROCEDURE GRABAR_CURSOS
(Codigo Cursos.Cod_Curso%Type, Descripcion Cursos.Descripcion%Type,
 Credito Cursos.Credito%Type, Estado Cursos.Estado%Type)
IS
BEGIN
      INSERT INTO CURSOS VALUES (Codigo, Descripcion, Credito, Estado);
      COMMIT;
END;


--  CONSULTAR
CREATE OR REPLACE PROCEDURE CONSULTAR_CURSOS
(Codigo CURSOS.Cod_Curso%Type)
IS
  Descripcion Cursos.Descripcion%Type;
  Credito Cursos.Credito%Type; Estado Cursos.Estado%Type;
BEGIN
     SELECT Descripcion, Credito, Estado
     INTO Descripcion, Credito, Estado FROM CURSOS
...
EXCEPTION
     WHEN NO_DATA_FOUND THEN
         DBMS_OUTPUT.PUT_LINE('EL CURSO NO ESTA REGISTRADO');
END;


--  ELIMINAR
CREATE OR REPLACE PROCEDURE ELIMINAR_CURSOS
(Codigo Cursos.Cod_Curso%Type)
IS
BEGIN
...
--  ACTUALIZAR
CREATE OR REPLACE PROCEDURE ACTUALIZAR_CURSOS
(Codigo Cursos.Cod_Curso%Type, Descripcion Cursos.Descripcion%Type,
 Credito Cursos.Credito%Type, Estado Cursos.Estado%Type)
IS
BEGIN
     UPDATE CURSOS SET Descripcion = Descripcion, Credito = Credito,
                       Estado = Estado
                       WHERE Cod_Curso = Codigo;
     COMMIT;
END;
...
--PROCEDIMIENTO PROFESOR
--  GRABAR
CREATE OR REPLACE PROCEDURE GRABAR_PROFESOR
(Codigo Profesor.Cod_Prof%Type, Nombre Profesor.Nombre%Type,
 Profesion Profesor.Profesion%Type, fechaNacimiento Profesor.F_Nac%Type,
 DniProfesor Profesor.Dni%TYPE, Sexo Profesor.Sexo%TYPE, 
 Edad Profesor.Edad%Type, Estado Profesor.Estado%Type)
IS
BEGIN
      INSERT INTO PROFESOR VALUES (Codigo, Nombre, Profesion, fechaNacimiento, DniProfesor, Sexo, Edad, Estado);
      COMMIT;
END;

--  CONSULTAR
CREATE OR REPLACE PROCEDURE CONSULTAR_PROFESOR
(Codigo Profesor.Cod_Prof%Type)
IS
  ...
  Edad Profesor.Edad%Type; Estado Profesor.Estado%Type;
BEGIN
     SELECT Nombre, Profesion, F_Nac, Dni, Sexo, Edad, Estado 
     INTO Nombre, Profesion, fechaNacimiento, DniProfesor, Sexo, Edad, Estado FROM PROFESOR
     ...
     DBMS_OUTPUT.PUT_LINE('Fecha Nacimiento : ' || fechaNacimiento);
     DBMS_OUTPUT.PUT_LINE('DNI              : ' || DniProfesor);
...
EXCEPTION
     WHEN NO_DATA_FOUND THEN
         DBMS_OUTPUT.PUT_LINE('EL PROFESOR NO ESTA REGISTRADO');
END;
...
IS
BEGIN
     DELETE FROM PROFESOR WHERE COD_PROF = Codigo;
     COMMIT;
END;
...
IS
BEGIN
     UPDATE PROFESOR SET Nombre = Nombre, Profesion = Profesion,
                       F_Nac = fechaNacimiento, Dni = DniProfesor, Sexo = Sexo,
                       Edad = Edad, Estado = Estado
                       WHERE Cod_Prof = Codigo;
     COMMIT;
END;
...
CREATE OR REPLACE PACKAGE MANTEMIENTO
IS
      FUNCTION SUELDO_MAX RETURN FLOAT;
      FUNCTION SUELDO_MIN RETURN FLOAT;
      FUNCTION TOTAL_HOMBRE RETURN INT;
      FUNCTION TOTAL_LIBROS RETURN NUMBER;
      FUNCTION TOTAL_LIBROS RETURN INT;
      PROCEDURE GRABAR_ALUMNO;
      PROCEDURE CONSULTAR_ALUMNO;
      PROCEDURE ELIMINAR_ALUMNO;
      PROCEDURE ACTUALIZAR_ALUMNO;
      
      PROCEDURE GRABAR_CURSOS;
      PROCEDURE CONSULTAR_CURSOS;
      PROCEDURE ELIMINAR_CURSOS;
      PROCEDURE ACTUALIZAR_CURSOS;
      
      PROCEDURE GRABAR_PROFESOR;
      PROCEDURE CONSULTAR_PROFESOR;
      PROCEDURE ELIMINAR_PROFESOR;
      PROCEDURE ACTUALIZAR_PROFESOR;
      
END;

CREATE OR REPLACE PACKAGE BODY MANTEMIENTO
IS
      -- FUNCIONES
      -- Función Sueldo Máximo
      FUNCTION SUELDO_MAX RETURN FLOAT;
